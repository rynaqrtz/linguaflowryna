# CONTRIBUTE.md — Catatan Perubahan (Frontend)

Dokumen ini merangkum semua perubahan yang saya kerjakan di sisi
frontend, **sementara backend/Supabase sedang dikerjakan terpisah**. Tujuannya
supaya siapa pun yang me-review (atau menyambungkan backend nanti) tahu persis
apa yang berubah, kenapa, dan apa yang masih perlu dilanjutkan.

Semua perubahan sudah dites dengan `npx tsc --noEmit` (bersih) dan
`npm run build` (34 halaman ter-generate tanpa error) di setiap tahap.
...
| AI Sensei terhubung ke Gemini 2.5 Flash (bukan demo) | ✅ Selesai (butuh API key asli) |
| Latihan Ucapan pakai Web Speech API asli (bukan demo) | ✅ Selesai |
| Auth sungguhan (Supabase Auth + middleware, ganti localStorage) | ✅ Selesai (butuh project Supabase asli) |
| Manajemen akun guru (undang lewat email) | ✅ Selesai (butuh Supabase asli) |
| Polish animasi landing page (Framer Motion tambahan) | ✅ Selesai (JourneyProgress) |
---
...
## Bagian 6 — Auth Sungguhan & Manajemen Akun

### Login/sesi sekarang 100% Supabase Auth, bukan lagi localStorage
- `src/lib/supabase/client.ts`, `server.ts`, `middleware.ts` — setup client
  Supabase standar untuk Next.js App Router.
- `middleware.ts` (root) — proteksi route yang SUNGGUHAN di server. Dulu
  `useRoleGuard` cuma jalan di client dan bisa dilewati lewat devtools;
  sekarang kalau belum login, request ke `/a`, `/g`, `/m` di-redirect ke
  `/login` SEBELUM halaman sempat ter-render.
- `src/lib/user-context.tsx` — `login()` sekarang `signInWithPassword`
  sungguhan (bukan cuma nebak role dari email), `signUp()` untuk
  registrasi, `logout()` benar-benar memanggil `supabase.auth.signOut()`.
- **RoleSwitcher dihapus total** (`src/components/layout/RoleSwitcher.tsx`
  + semua pemakaiannya di AppSidebar/StudentSidebar/profil murid). Tombol
  "ganti role instan tanpa login ulang" itu secara fundamental gak bisa
  ada lagi begitu auth-nya sungguhan — kalau masih ada, itu sendiri jadi
  lubang keamanan besar.
- Ketemu 1 lagi bug hardcode nama pas beres-beres ini:
  `StudentSidebar.tsx` (sidebar desktop murid) ternyata masih hardcode
  "Ahmad Fauzi" — kelewat dari pembersihan sebelumnya karena beda file
  dari `StudentShell.tsx`. Sudah diganti pakai `useUser().name`.

### Cara bikin akun admin pertama kali
Alur yang sudah ada: buka **`/register-sekolah`** — ini bukan cuma daftar
sekolah, tapi sekaligus bikin akun admin pertama untuk sekolah itu (lihat
`src/app/api/register-school/route.ts` + `signUp({ role: "admin" })` di
`RegisterSekolahClient.tsx`). Jadi urutannya:
1. Jalankan `supabase/schema.sql` + `seed.sql` di project Supabase kamu
2. Buka `/register-sekolah`, isi nama sekolah + data akun kamu sendiri
3. Itu otomatis jadi akun admin kamu — gak perlu di-edit manual di
   database, KECUALI kalau mau assign school_id ke sekolah yang sudah ada
   sebelumnya (bukan bikin baru), itu baru perlu update manual lewat SQL
   Editor Supabase (`update profiles set role = 'admin' where id = ...`).

### Cara admin nambah akun guru
`/a/guru` → "Tambah Guru" sekarang beneran mengirim email undangan lewat
`src/app/api/invite-teacher/route.ts` (pakai
`supabase.auth.admin.inviteUserByEmail`, guru dapat email buat set
password sendiri, bukan admin yang set password-kan). Route ini ngecek
dulu yang request beneran admin (`role === "admin"` dari sesi login),
jadi gak bisa dipanggil sembarangan.

### Murid tetap daftar sendiri lewat kode kelas
`/register` (alur "Gabung ke Kelas") tidak berubah — tetap `signUp({
role: "murid" })`, jadi default role publik memang selalu "murid", sesuai
yang ditanyakan. Cuma admin (lewat `/register-sekolah`) dan guru (lewat
undangan admin) yang bisa dapat role lain.

### AI Sensei — sekarang bisa 2 API key (failover)
`src/app/api/sensei/route.ts` sekarang nyoba `GEMINI_API_KEY` dulu, kalau
kena rate limit/kuota habis (HTTP 429 atau status `RESOURCE_EXHAUSTED`),
otomatis coba `GEMINI_API_KEY_2` tanpa user perlu tahu ada retry di
belakang layar. Model yang dipakai: `gemini-2.5-flash` (dari 3 model yang
kamu kasih, ini yang cocok buat chat teks — 2 lainnya,
`gemini-2.5-flash-native-audio-preview` dan `gemini-2.5-flash-preview-tts`,
itu buat audio real-time & text-to-speech, beda use-case dari chat AI
Sensei. Kalau nanti mau upgrade "Dengar Contoh Native" di halaman Latihan
Ucapan dari `speechSynthesis` browser ke Gemini TTS asli, itu bisa jadi
kerjaan terpisah.
